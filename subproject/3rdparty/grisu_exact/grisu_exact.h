...
namespace jkj {
	namespace grisu_exact_detail {
		...
		template <class UInt>
		static inline bool divisible_by_power_of_5(UInt x, unsigned int exp) noexcept {
			static const auto table = generate_divisibility_test_table<UInt,
				std::is_same<UInt, std::uint32_t>::value ? 12 : 24>();
			assert(exp < table.size);
			auto const& entry = table.value[exp];
		...
		struct cache_holder<float> {
			static inline const std::uint64_t& cache(std::size_t index) noexcept {
		...
		struct cache_holder<double> {
			static inline const uint128& cache(std::size_t index) noexcept {
				static const uint128 table[] = {
		...
		template <class Float>
		inline typename common_info<Float>::cache_entry_type const& get_cache(int k) noexcept {
